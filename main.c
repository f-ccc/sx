...
static int load_data(void)
{
    ...
    ret = csv_load_records(DATA_FILE, g_records, &g_record_count, g_record_capacity);
    if (ret != OK) {
...
static int save_data(void)
{
    ...
    ret = csv_save_records(DATA_FILE, g_records, g_record_count);
    if (ret != OK) {
        printf("错误：保存数据失败\n");
        return ERR;
    }
    
    printf("数据已保存至 %s (共 %d 条记录)\n", DATA_FILE, g_record_count);
    return OK;
}
...
int main(void)
{
    ...
    /* 尝试加载已有数据 */
    {
        g_records = (Record *)malloc(sizeof(Record) * MAX_RECORDS);
        if (g_records != NULL) {
